 ...
 * Author: nadia
 *
 * Created on October 18, 2022, 7:20 PM
 */
...
void InitUART2(void) 
{
	...
	TRISBbits.TRISB0=0; // Set U2TX/RB0/pin 4 as UART TX output (transmitting)
	TRISBbits.TRISB1=1; // Set U2RX/RB1/pin 5 as UART RX  input (receiving))
	LATBbits.LATB0=1; // High output 
 ...
 */
    ...
    // To make sure the microcontroller and computer have same  speed 
    // Sets baud rate based on current microcontroller's clock
...
// Takes a character you want to send to the computer and how many times you want to send it out 
...
{	
   // InitUART2();	//Initialize UART2 module at start of main()
	...
	while(repeatNo!=0) 
	{
		...
		{
			Idle(); // once there is space in buffer, interrupt gets trigger
		}	
...
// Interrupt when computer sends data to microcontroller. Once data hits rx port, it trgigers an interrupt.
//void __attribute__ ((interrupt, no_auto_psv)) _U2RXInterrupt(void) {
//    IFS1bits.U2RXIF = 0;
//    U2RXREG = 0; // Moving data that's coming from the computer to the next buffer
//    U2RXREG = 0;
//    U2RXREG = 0;
//    U2RXREG = 0;
//    U2RXREG = 0;
//    RxChar = U2RXREG; // Out of buffer
//    Nop();
//    Nop();
//    return;
//
...
// Takes in a 16 bit wide number and displays it on the pc terminal using hex format
