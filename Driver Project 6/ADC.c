...
void display_graph(uint16_t ADC_VALUE)
{
    ...
    float resolution = (float)5/1024; // Calculating the resolution
    float VIN = ADC_VALUE * resolution; // Using the resolution and ADC value to find the input voltage
    
    Disp2Dec(ADC_VALUE); // Transmitting the ADCBUF value via UART
    char str[10];
    sprintf(str, "%.3f", VIN); // Converting the VIN value to a string
    Disp2String(str); // Transmitting the VIN value via UART
    XmitUART2('\n', 1); // Newline to signal the end of a pair of data (buffer value, voltage)
    
    NewClk(32); // Change clock back to 32 Khz
